...
public class ModRecipeProvider extends FabricRecipeProvider {
    ...
    public static void offerToolRecipe(Consumer<RecipeJsonProvider> exporter, Item ingredient, Item tool, ToolType type) {
        ShapedRecipeJsonBuilder.create(RecipeCategory.TOOLS, tool, 1)
                ...
                .criterion(RecipeProvider.hasItem(tool), RecipeProvider.conditionsFromItem(tool))
                .offerTo(exporter);
    }
    public static void offerSwordRecipe(Consumer<RecipeJsonProvider> exporter, Item ingredient, Item sword) {
        offerToolRecipe(exporter, ingredient, sword, ToolType.SWORD);
    }
    public static void offerPickaxeRecipe(Consumer<RecipeJsonProvider> exporter, Item ingredient, Item pickaxe) {
        offerToolRecipe(exporter, ingredient, pickaxe, ToolType.PICKAXE);
    }
    public static void offerAxeRecipe(Consumer<RecipeJsonProvider> exporter, Item ingredient, Item axe) {
        offerToolRecipe(exporter, ingredient, axe, ToolType.AXE);
    }
    public static void offerShovelRecipe(Consumer<RecipeJsonProvider> exporter, Item ingredient, Item shovel) {
        offerToolRecipe(exporter, ingredient, shovel, ToolType.SHOVEL);
    }
    public static void offerHoeRecipe(Consumer<RecipeJsonProvider> exporter, Item ingredient, Item hoe) {
        offerToolRecipe(exporter, ingredient, hoe, ToolType.HOE);
    }
    public static void offerToolsRecipe(
        ...
        if(sword != null) offerSwordRecipe(exporter, ingredient, sword);
        if(pickaxe != null) offerPickaxeRecipe(exporter, ingredient, pickaxe);
        if(axe != null) offerAxeRecipe(exporter, ingredient, axe);
        if(shovel != null) offerShovelRecipe(exporter, ingredient, shovel);
        if(hoe != null) offerHoeRecipe(exporter, ingredient, hoe);
    }
}
