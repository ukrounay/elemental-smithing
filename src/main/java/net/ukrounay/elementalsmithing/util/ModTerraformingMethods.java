...
public class ModTerraformingMethods {
    ...
    public static void startTerraforming(World world, BlockPos pos, Block initialBlock, Block curveBlock, int ticks, int stage) {
        ...
        world.playSound(null, pos, ModSounds.TERRAFORMING_START, SoundCategory.BLOCKS, 1f, random.nextFloat());
     ...
     */
    public static void prepareCurlySpikes(World world, Block initialBlock, Block curveBlock) {
        ...
        world.playSound(null, startPos, ModSounds.TERRAFORMING_START, SoundCategory.BLOCKS, 1f, random.nextFloat());
    }
...
//    }
     ...
     */
    public static void startRestoration(World world, int restoringTicks) {
        isRestoring = true;
        maxRestoringTicks = restoringTicks;
        remainingRestoringTicks = maxRestoringTicks;
        world.playSound(null, startPos, ModSounds.TERRAFORMING_START, SoundCategory.BLOCKS, 1, 1);
    }
        ...
        for (int x = -radiusX; x <= radiusX; x++)
            for (int y = -radiusY; y <= radiusY; y++)
                for (int z = -radiusZ; z <= radiusZ; z++) {
                    ...
                    if (distanceSquared < 1.0 && !blacklist.contains(world.getBlockState(blockPos).getBlock()))
